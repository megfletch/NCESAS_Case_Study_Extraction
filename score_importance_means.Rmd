...
means_merged_by_case <- cbind(aguion_mean, burden_mean, dickey_collas_mean, eurich_mean, free_mean, friedman_mean, kleisner_mean, lau_mean, mason_mean, mills_mean, pecl_mean, tokunaga_mean, westfall_mean, zhao_mean) %>% 
  select(dimension...1, domain...2, Aguion, Burden, Dickey_Collas, Eurich, Free, Friedman, Kleisner, Lau, Mason, Mills, Pecl, Tokunaga, Westfall, Zhao) %>% 
...
```{r}
# Merge all mean datasets together
importance_merged_by_case <- cbind(aguion_importance, burden_importance, dickey_collas_importance, eurich_importance, free_importance, friedman_importance, kleisner_importance, lau_importance, mason_importance, mills_importance, pecl_importance, tokunaga_importance, westfall_importance, zhao_importance) %>% 
  select(dimension...1, domain...2, Aguion, Burden, Dickey_Collas, Eurich, Free, Friedman, Kleisner, Lau, Mason, Mills, Pecl, Tokunaga, Westfall, Zhao) %>% 
  rename(Dimension = dimension...1, Domain = domain...2)
...
# Plot Score Means vs. Importance Means

```{r}
# These means are different (ie. based on 1-4 and 1-3) so putting them next to each other on a bar plot doesn't amke much sense
```
...
small_scale_score <- attributes_merged_clean %>% 
  filter(case_study %in% c("Aguion", "Eurich", "Friedman", "Kleisner", "Lau", "Pecl", "Tokunaga", "Zhao")) %>% 
  group_by(attribute) %>% 
...
small_scale_combined <- merge(small_scale_score, small_scale_importance) %>% 
  rename(Attributes = attribute,
         "Score Mean." = score..1,
         "Score SD." = score..2,
         "Importance Mean." = numeric_importance..1,
         "Importance SD." = numeric_importance..2)
```
...
scale_merged <- merge(small_scale_combined, large_scale_combined)
```


## Small-Scale Fisheries Plot

```{r}
ggplot() +
  geom_bar(data = scale_merged, stat = 'identity', mapping = aes(x = Attributes, y = "Score Mean."))
```
