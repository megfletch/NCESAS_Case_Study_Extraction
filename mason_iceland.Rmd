...
mason_eco_mean <- (data = mason_ecological_scores) %>% 
  select(score_1_4) 
mason_eco_mean <- lapply(mason_eco_mean, as.numeric)

mason_socio_mean <- (data = mason_socioeconomic_scores) %>% 
  select(score_1_4) 
mason_socio_mean <- lapply(mason_socio_mean, as.numeric)

mason_gov_mean <- (data = mason_governance_scores) %>% 
  select(score_1_4) 
mason_gov_mean <- lapply(mason_gov_mean, as.numeric)


mean(mason_eco_mean$score_1_4) #3
mean(mason_socio_mean$score_1_4)
mean(mason_gov_mean$score_1_4) #3

```

```{r}
#Capacity scores by attribute and importance
ggplot(mason_iceland, aes(x = resilience_attribute, y = score_1_4, fill = importance_of_attribute_in_study_system_high_medium_low, group_by = dimensions)) +
  geom_col() +
...
```{r}
#Make plots by dimension
mason_eco_plot <- ggplot(mason_ecological_scores, aes(x = resilience_attribute, y = score_1_4, fill = importance_of_attribute_in_study_system_high_medium_low)) +
  geom_col() +
...
mason_socio_plot <- ggplot(mason_socioeconomic_scores, aes(x = resilience_attribute, y = score_1_4, fill = importance_of_attribute_in_study_system_high_medium_low)) +
  geom_col() +
...
mason_gov_plot <- ggplot(mason_governance_scores, aes(x = resilience_attribute, y = score_1_4, fill = importance_of_attribute_in_study_system_high_medium_low)) +
  geom_col() +
