...
# Find Importance Score Means 
The importance score means of each attribute are reflected in the table. These means are based off of all case studies combined.In onder to calculate means, the importance levels needed to converted to numeric value so, _high importance = 3, medium = 2, and low = 1_.
...
merged_importance %>% 
  kbl(caption = "Importance Score Means") %>% 
...
dimension_importance %>% 
  kbl(caption = "Importance Score Means by Dimension") %>% 
   kable_classic_2(full_width = F, html_font = "Cambria") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
# Create a table
domain_importance %>% 
  kbl(caption = "Importance Score Means by Domain") %>% 
   kable_classic_2(full_width = F, html_font = "Cambria") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
# Grouping Importance Score Stats

## Importance Score Means by Dimension
```{r}
...
dimension_importance_merged <- cbind(aguion_dim, burden_dim, dickey_collas_dim, eurich_dim, free_dim, friedman_dim, hollowed_dim,  kleisner_dim, lau_dim, mason_dim, mills_dim, pecl_dim, tokunaga_dim, westfall_dim, zhao_dim) %>% 
...
dimension_importance_merged %>% 
  kbl(caption = "Importance Score Means Across by Dimension") %>% 
   kable_classic_2(full_width = F, html_font = "Cambria") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"))
```


## Importance Score Means by Domain
...
# Merge all mean datasets together
means_merged_by_case <- cbind(aguion_dom, burden_dom, dickey_collas_dom, eurich_dom, free_dom, friedman_dom, hollowed_dom,  kleisner_dom, lau_dom, mason_dom, mills_dom, pecl_dom, tokunaga_dom, westfall_dom, zhao_dom) %>% 
...
means_merged_by_case %>% 
  kbl(caption = "Importance Score Means Across Case Studies") %>% 
   kable_classic_2(full_width = F, html_font = "Cambria") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
# Create a table
importance_stats %>% 
  kbl(caption = "Proportion of Importanc Levels by each Dimension") %>% 
   kable_classic_2(full_width = F, html_font = "Cambria") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"))
```
